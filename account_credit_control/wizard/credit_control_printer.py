...
class CreditControlPrinter(models.TransientModel):
    ...
    def print_lines(self):
        ...
        report_name = 'account_credit_control.credit_control_summary'
        ctx = dict(self.env.context, active_ids=comms.ids, active_model=comms._name)

        return self.env.ref(report_name).with_context(ctx).report_action(comms)
